...
fn main() {
    ...
    //    let phases_combinations = generate_combinations(5, 9);
    let phases_combinations = vec![vec![9, 7, 8, 5, 6]];
    for phases in phases_combinations {
        for phase in dbg!(phases) {
            input = parse(opcodes.clone(), vec![phase, input]);
        }
    ...
    println!("FINAL OUTPUT: {}", max_thrust);
}
